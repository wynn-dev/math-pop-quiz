    ...
    <transition name="fade" mode="out-in" v-if="!loading">
      <div :key="currentQuestion">
        ...
        <span
          ...
          v-if="stage === 'results'"
          @click="toggleModal"
        >Clique aqui para a explicação</span>
        <!-- Modal -->
        <explanation-modal
  ...
  computed: {
    ...
    resultsInfo() {
      if (this.correctAnswers < 2) {
        return {
          text: "Casos notaveis é um presunto <br>E tu não es melhor do que um presunto",
          img: require("../assets/kumamon.png")
        }
      }
      if (this.correctAnswers < 4) {
        return {
          text: "Nunca desista, tal como Rick Astley faz.",
          img: require("../assets/rick-roll.gif")
        }
      }
      if (this.correctAnswers < 6) {
        return {
          text: "Porque é tão difícil acabar com uma namorada japonesa? Porque é preciso largar a bomba duas vezes antes que ela a receba.",
          img: require("../assets/ricardo.gif")
        }
      }
      if (this.correctAnswers < 8) {
        return {
          text: 'Uma vez entrei num bar. Magoou-me muito a cabeça.',
          img: require("../assets/kid.gif")
        }
      }
      if (this.correctAnswers < 10) {
        return {
          text: 'V2h5IGRpZCB5b3UgZXZlbiB0cnk=',
          img: require("../assets/kid.gif")
        }
      }
      if (this.correctAnswers > 12) {
        return {
          text: "Um grande matematico como você, é como um pirilampo no escuro, tão brilhante, tão distinto, não importa onde esteja.",
          img: require("../assets/obtive.gif")
        }
      } else {
        return {}
      }
